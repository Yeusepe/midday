...
description: Create applications that integrate with Creator Payments using OAuth 2.0.
section: developer
order: 2
---

Build applications that let users connect their Creator Payments accounts. OAuth apps can access financial data on behalf of users, enabling integrations, automations, and custom tools.

## What you can build

With OAuth, you can build:

- **Browser extensions** that display Creator Payments data
- **Mobile apps** with Creator Payments integration
...
- **Public apps** listed in the Creator Payments app directory

## How OAuth works

OAuth 2.0 lets your app request access to a user's Creator Payments data without handling their password:

1. Your app redirects users to Creator Payments' authorization page
2. Users log in and approve the requested permissions
3. Creator Payments redirects back with an authorization code
4. Your app exchanges the code for access tokens
5. Use access tokens to call the Creator Payments API
...
After the user authorizes, Creator Payments redirects to your `redirect_uri`:
...
```bash
curl -X POST https://api.payments.yucp.club/v1/oauth/token \
...
Install the Creator Payments SDK:
...
```typescript
import { Creator Payments } from "@midday-ai/sdk";
...
```bash
curl -X POST https://api.payments.yucp.club/v1/oauth/revoke \
  -H "Content-Type: application/json" \
  -d '{
    "token": "mid_at_xxxxx",
    "client_id": "YOUR_CLIENT_ID",
...
app.get("/callback", async (req, res) => {
  ...
  const response = await fetch("https://api.payments.yucp.club/v1/oauth/token", {
    method: "POST",
