---
title: "Creator Payments Computer: Autonomous Agents for Your Business"
publishedAt: "2026-04-15"
summary: "Introducing the private beta of Creator Payments Computer. Describe what you need in plain English and Creator Payments deploys an autonomous agent that runs on your schedule, learns over time, and delivers results while you sleep."
tag: "Updates"
---

We've been building Creator Payments as an open-source business operating system. Banking, invoicing, transactions, time tracking, all in one place. Over the past year, we've made all of that accessible through 100+ MCP tools, powering the dashboard chat, the iMessage bot, and the public API.
...
Today we're opening the **private beta** of **Creator Payments Computer**. Describe what you need, and Creator Payments deploys an autonomous agent that runs on your schedule, learns over time, and delivers results while you sleep.
...
You describe the outcome. Creator Payments handles the rest.

<br />

### Works wherever you are

You don't need to learn a new tool. Creator Payments Computer meets you where you already work:
...
Most automations are stateless. They do the same thing every time. Creator Payments Computer agents have **persistent memory** that compounds across runs:
...
We think trust is the product when it comes to agents handling your business data. That's why Creator Payments Computer has approval mode built in:
...
Or describe your own workflow and Creator Payments generates a custom agent. Type-checked, compiled, and deployed in seconds.

<br />

### How it works under the hood

When you describe a workflow, Creator Payments:
...
**The tool surface we already built is the foundation.** Every piece of Creator Payments data is already accessible through well-typed tools. Creator Payments Computer is the first consumer of that surface that isn't a human in a chat window. Every tool we add for the assistant automatically becomes available to every agent. The investment compounds.
...
Creator Payments Computer is launching in **private beta**. We're onboarding teams gradually to make sure agents are reliable, the catalog is tuned for real-world usage, and we can iterate closely with early users.
...
Creator Payments Computer is open source, like everything else we build. The code is at [github.com/midday-ai/midday](https://github.com/midday-ai/midday).
