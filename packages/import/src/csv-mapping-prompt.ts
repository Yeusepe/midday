...
export function buildCsvMappingPrompt(
  ...
  return [
    ...
    "8) If separate debit/credit columns exist and a generic Amount column also exists, map amount to Amount. Never map amount to only Debit or only Credit when a combined Amount column is available.",
    "</rules>",
    "",
    "<examples>",
    "<example>",
    ...
    "</example>",
    "<example>",
    "<columns>['Date','Description','Debit','Credit','Amount']</columns>",
